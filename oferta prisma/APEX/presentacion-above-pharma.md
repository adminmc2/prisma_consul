# PRESENTACIÓN COMERCIAL — ABOVE PHARMA

**Folio:** PC26-0205
**Fecha:** 5 de febrero de 2026
**Validez:** 30 días

---

## INSTRUCCIONES DE DISEÑO PARA KIMI

**IMPORTANTE:** Respetar estos elementos en TODAS las diapositivas.

### PALETA DE COLORES
- **Fondo principal:** Azul oscuro (#0a1628 o similar)
- **Texto principal:** Blanco (#FFFFFF)
- **Acentos/destacados:** Azul claro/cyan (#00D4FF o similar)
- **Tablas:** Fondo semi-transparente con bordes redondeados
- **Precios/números importantes:** Color cyan para destacar

### TIPOGRAFÍA
- **Títulos:** Sans-serif bold, tamaño grande (mínimo 36pt)
- **Subtítulos:** Sans-serif medium (24-28pt)
- **Cuerpo:** Sans-serif regular (16-18pt)
- **Números/precios:** Tamaño mayor, destacados en negrita

### LOGOS
- **Logo PRISMA:** Esquina inferior derecha en TODAS las diapositivas
- **Logo Above Pharma:** Portada y diapositivas de cierre

### ELEMENTOS VISUALES
- Iconos minimalistas y modernos para cada sección
- Líneas divisorias sutiles (blancas semi-transparentes)
- Espaciado generoso entre elementos
- Cajas/tarjetas con bordes redondeados y sombra sutil
- QR con marco visible y URL debajo

### ESTILO GENERAL
- Diseño limpio y profesional
- Aspecto tecnológico/moderno
- Jerarquía visual clara
- Consistencia en todas las diapositivas

---

## DIAPOSITIVA 1: PORTADA

ABOVE PHARMA
Transformación del Ecosistema Digital

Estrategia de Marca + Automatización de Procesos + Inteligencia Artificial

Febrero 2026

---

## DIAPOSITIVA 2: EL RETO

EL RETO

Above Pharma necesita acelerar su presencia en el mercado mexicano y latinoamericano.

¿Cómo lo logramos?

- Marca posicionada estratégicamente
- Un sistema que automatiza tus procesos y experiencias
- Asistente IA que multiplica la efectividad de ventas

Un ecosistema integral, no herramientas sueltas.

---

## DIAPOSITIVA 3: LA SOLUCIÓN — 3 PILARES

NUESTRA PROPUESTA
3 pilares integrados

| BRAND PLAN | APEX | OMIA |
|------------|------|------|
| Estrategia MyFiller | Automatización de Procesos y EXperiencias | Asistente IA Ventas |
| 3.300€ / $66,000 MXN | ~~5.000€~~ **2.500€** / $50,000 MXN | ~~1.000€~~ **500€** / $10,000 MXN |

ECOSISTEMA ABOVE

PAQUETE INTEGRAL: 5.670€ / $113,400 MXN
(Incluye 50% dto. APEX + 50% dto. OMIA + 10% dto. paquete = Ahorras 3.630€)

---

## DIAPOSITIVA 4: BRAND PLAN — ¿QUÉ ES?

BRAND PLAN
Estrategia MyFiller

Plan estratégico integral para posicionar MyFiller en México y Latinoamérica.

Rellenos Dérmicos de Ácido Hialurónico

No es solo un documento.
Es tu HOJA DE RUTA para conquistar el mercado.

Define QUÉ decir, A QUIÉN, y CÓMO posicionarte.

---

## DIAPOSITIVA 5: BRAND PLAN — 8 RUBROS

BRAND PLAN: 8 Rubros Estratégicos

1. DIAGNÓSTICO Y ANÁLISIS DE MERCADO
   Mercado de medicina estética, competencia, FODA

2. ESTRATEGIA DE POSICIONAMIENTO
   Propuesta de valor, mensajes clave por audiencia

3. PROGRAMA KOLs Y MEDICAL ADVOCACY
   Identificación de KOLs, programa embajadores, Speaker Training

4. PLAN DE MARKETING Y COMUNICACIÓN
   Lanzamiento, congresos, marketing digital

5. FORMACIÓN Y GESTIÓN FUERZA DE VENTAS
   Capacitación, herramientas, manejo de objeciones

6. ESTRATEGIA COMERCIAL
   Modelo de precios, política de descuentos

7. PLAN FINANCIERO Y PROYECCIONES
   Proyección de ventas, presupuesto, KPIs

8. ROADMAP DE IMPLEMENTACIÓN
   Cronograma maestro, fases, milestones

INVERSIÓN: 3.300€ / $66,000 MXN

---

## DIAPOSITIVA 6: BRAND PLAN — DIAGNÓSTICO Y POSICIONAMIENTO

BRAND PLAN MyFiller
Diagnóstico + Posicionamiento

1. DIAGNÓSTICO Y ANÁLISIS DE MERCADO

- Tamaño y crecimiento del mercado de rellenos dérmicos en México/LATAM
- Análisis de competencia: Juvederm, Restylane, Radiesse, Sculptra
- Matriz de posicionamiento competitivo
- Análisis FODA de MyFiller

2. ESTRATEGIA DE POSICIONAMIENTO

- Propuesta de valor diferencial de MyFiller
- Posicionamiento por línea de producto
- Atributos clave a comunicar
- Mensajes diferenciados:
  → Para médicos: evidencia clínica, facilidad de uso
  → Para pacientes: resultados naturales, seguridad

---

## DIAPOSITIVA 7: BRAND PLAN — KOLs Y MARKETING

BRAND PLAN MyFiller
KOLs + Marketing

3. PROGRAMA KOLs Y MEDICAL ADVOCACY

- Identificación y mapeo de KOLs en México/LATAM
- Estructura del programa embajadores
- Modelo de colaboración y compensación
- Speaker Training (formación de ponentes)
- Plan de actividades con KOLs

4. PLAN DE MARKETING Y COMUNICACIÓN

- Plan integral de lanzamiento
- Educación médica continua
- Workshops y webinars
- Congresos y eventos propios
- Marketing omnicanal y digital
- Estrategia de email y sampling

---

## DIAPOSITIVA 8: BRAND PLAN — VENTAS Y ESTRATEGIA

BRAND PLAN MyFiller
Ventas + Estrategia Comercial

5. FORMACIÓN Y GESTIÓN FUERZA DE VENTAS

- Programa de capacitación para el equipo
- Contenido técnico-científico
- Herramientas de ventas (sales aids)
- Manejo de objeciones
- Plan de onboarding nuevos reps

6. ESTRATEGIA COMERCIAL

- Modelo de precios por producto y canal
- Política comercial (descuentos, bonificaciones, condiciones)
- Programas de incentivos para fuerza de ventas

---

## DIAPOSITIVA 9: BRAND PLAN — FINANCIERO Y ROADMAP

BRAND PLAN MyFiller
Financiero + Implementación

7. PLAN FINANCIERO Y PROYECCIONES

- Proyección de ventas por escenarios
- Presupuesto de marketing por actividad
- Definición de KPIs financieros y comerciales

8. ROADMAP DE IMPLEMENTACIÓN

- Cronograma maestro de implementación
- 4 fases: Pre-lanzamiento → Lanzamiento → Consolidación → Expansión
- Milestones y checkpoints de revisión
- Quick wins y acciones inmediatas

INVERSIÓN BRAND PLAN: 3.300€ / $66,000 MXN

---

## DIAPOSITIVA 10: APEX — ¿QUÉ ES?

APEX
Automatización de Procesos y EXperiencias

"Tu Sistema, Tu Ritmo"

APEX es un sistema líquido de gestión comercial que SE ADAPTA A TI, no al revés.

No te vendemos un producto genérico.
Te ayudamos a construir TU sistema,
empezando por lo que MÁS te duele.

¿La diferencia? Es un SISTEMA LÍQUIDO: como el agua que toma la forma de su recipiente.

---

## DIAPOSITIVA 11: APEX — EL PROBLEMA QUE RESUELVE

APEX: El Problema

¿Te suena familiar?

"No sé qué hace mi equipo en campo"

"Los reportes me toman 2 días"

"Todo está en Excel y cada quien tiene su propio archivo"

"Los vendedores no usan el sistema actual"

"No tengo visibilidad de mis médicos"

Otros sistemas fallan porque te obligan a adaptarte a ellos.
APEX se adapta a tu forma de trabajar.

---

## DIAPOSITIVA 12: APEX — SISTEMA LÍQUIDO

APEX: El Sistema Líquido

| Sistemas Tradicionales | APEX |
|------------------------|------|
| Módulos rígidos que compras | Capacidades que se adaptan |
| "Esto no está incluido" | Todo es posible |
| Cambiar algo rompe lo demás | Añadir es natural |
| Te adaptas al software | El software se adapta a ti |

¿Qué significa "líquido"?

Tu empresa es única. Tus procesos son únicos. Tus dolores son únicos.
El sistema se moldea a TU realidad.

---

## DIAPOSITIVA 13: APEX — DOS CAPAS DE TRABAJO

APEX: Dos Capas que Trabajan Juntas

CAPA 1: INTERFAZ ESTRUCTURADA
(Siempre disponible, incluida en TODOS los planes)

- Fichas de médicos, clientes, productos
- Listados de visitas, pedidos, muestras
- Formularios para registro
- Dashboards con métricas
- Reportes configurados

CAPA 2: LIENZO IA
(Varía según el plan de suscripción)

- Registro por voz → dictas, el sistema completa
- Consultas IA → preguntas, el sistema responde
- Briefings → resumen antes de cada visita
- Alertas inteligentes → avisa cuando algo está mal
- Agentes autónomos → ejecutan tareas solos

La Interfaz Estructurada NO consume IA.
El Lienzo IA consume según tu plan.

---

## DIAPOSITIVA 14: APEX — EXPERIENCIAS POR ROL

APEX: Una Experiencia por Rol

Cada tipo de usuario ve exactamente lo que necesita:

REPRESENTANTE
"¿Qué hago hoy y cómo registro en 60 segundos?"
→ Agenda del día, registro por voz, offline

SUPERVISOR
"¿Cómo va mi equipo y dónde intervenir?"
→ Vista de equipo, alertas, coaching

DIRECTOR
"¿El negocio va bien? ¿Qué riesgos hay?"
→ Dashboard nacional, tendencias, predicciones

COMERCIAL
"¿Cómo van mis ventas?"
→ Pipeline, cobranza, metas

ADMINISTRADOR
"¿Cómo configuro sin depender de TI?"
→ Configuración visual, sin programar

---

## DIAPOSITIVA 15: APEX — CONSTRUCCIÓN INICIAL

APEX: Tu Inversión Inicial

┌───────────────────────────────────────────────────────────────┐
│  Precio normal:           5.000€ / $100,000 MXN               │
│  OFERTA ESPECIAL:         2.500€ / $50,000 MXN                │
│  (50% descuento por lanzamiento)                              │
└───────────────────────────────────────────────────────────────┘

DESGLOSE:

┌───────────────────────────────────────────────────────────────┐
│  ESTRUCTURA BASE                    900€ / $18,000 MXN        │
│  Usuarios ilimitados, roles, interfaces,                      │
│  web + móvil, tu identidad, motor IA listo                    │
├───────────────────────────────────────────────────────────────┤
│  TUS 4 DOLORES                    1.600€ / $32,000 MXN        │
│  Crédito para resolver tus 4 problemas                        │
│  prioritarios con SOLUCIÓN COMPLETA                           │
├───────────────────────────────────────────────────────────────┤
│  TOTAL CON DESCUENTO              2.500€ / $50,000 MXN        │
│  (Ahorras 2.500€ / $50,000 MXN)                               │
└───────────────────────────────────────────────────────────────┘

NO son "módulos sueltos".
A cada dolor le damos CUERPO COMPLETO.

---

## DIAPOSITIVA 16: APEX — CÓMO FUNCIONAN LOS 4 DOLORES

APEX: Tus 4 Dolores

"Cuéntanos tus 4 mayores dolores. Tienes 1.600€ ($32,000 MXN) de crédito para resolverlos."

Por cada dolor configuramos:

| Elemento | Qué hacemos |
|----------|-------------|
| Entidades | Los datos que necesitas (médicos, visitas, muestras) |
| Flujos | Cómo trabaja tu equipo |
| Reportes | Las métricas que te importan |
| IA | Sugerencias y alertas del contexto |
| Integraciones | Conexiones básicas necesarias |

METODOLOGÍA:
1. Escuchamos tu dolor (con tus palabras)
2. Diseñamos la solución COMPLETA
3. Definimos qué construimos ahora
4. Mostramos qué más puede crecer después

---

## DIAPOSITIVA 17: APEX — EJEMPLO CONCRETO

APEX: Ejemplo para Above Pharma

TU DOLOR:
"No sé qué hacen mis reps cada día"

SOLUCIÓN COMPLETA QUE DISEÑAMOS:
Registro de visitas por voz + historial por médico + geolocalización + alertas de inactividad + IA que detecta patrones + predicción de cobertura + dashboard de supervisión

LO QUE CONSTRUIMOS CON EL CRÉDITO:
- Registro de visitas (móvil, offline, voz)
- Historial completo por médico
- IA que resume la actividad del día
- Dashboard básico de supervisión

POTENCIALIDADES (cuando quieras crecer):
→ Geolocalización en tiempo real
→ Alertas automáticas de inactividad
→ IA predictiva de cobertura
→ Dashboard avanzado con mapas

---

## DIAPOSITIVA 18: APEX — CUÉNTANOS TUS DOLORES (QR)

APEX para Above Pharma
Cuéntanos tus 4 Dolores

Para personalizar APEX necesitamos conocer qué te duele más.

Escanea el QR y completa el formulario:

┌─────────────────────────────────────────────────────┐
│                                                     │
│                    [CÓDIGO QR]                      │
│                                                     │
│           prismaconsul.com/apex             │
│                                                     │
└─────────────────────────────────────────────────────┘

El formulario:
- Te toma 5 minutos
- Detecta automáticamente tus dolores principales
- Te pedimos un audio con tus palabras
- En 48 horas recibes propuesta personalizada

---

## DIAPOSITIVA 19: APEX — SUSCRIPCIÓN: QUÉ INCLUYE

APEX: Suscripción Mensual

Después de construir el sistema, pagas una suscripción mensual por usuario.

QUÉ INCLUYE LA SUSCRIPCIÓN:

| Concepto | Qué significa |
|----------|---------------|
| Uso de la Plataforma | Acceso completo, hosting incluido |
| Actualizaciones | Siempre la última versión |
| Soporte Técnico | Atención cuando algo no funcione |
| IA Integrada | Según el plan que elijas |

TODOS los planes incluyen:
- Interfaz estructurada COMPLETA (fichas, listas, reportes)
- Usuarios ilimitados en tu cuenta
- Web + móvil + offline
- Soporte técnico
- **Mantenimiento incluido:** 10 tickets mensuales + atención WhatsApp

Lo que VARÍA entre planes es el acceso a IA.

---

## DIAPOSITIVA 20: APEX — LOS 3 PLANES DE SUSCRIPCIÓN

APEX: Tú Eliges Cuánta IA

| Capacidad | BASE 9€/mes ($180 MXN) | ESENCIAL 19€/mes ($380 MXN) | PRO 29€/mes ($580 MXN) |
|-----------|------------------------|----------------------------|------------------------|
| Interfaz estructurada | ✓ Completa | ✓ Completa | ✓ Completa |
| Fichas, listas, reportes | ✓ | ✓ | ✓ |
| Consultas IA / día | ❌ | 30 | 100 |
| Dictado por voz / día | ❌ | 10 | 30 |
| Lienzo IA | ❌ | Reactivo | Proactivo |
| Briefings automáticos | ❌ | ❌ | ✓ |
| Alertas inteligentes | ❌ | ❌ | ✓ |
| Agentes autónomos | ❌ | ❌ | 2 activos |
| Reportes IA / mes | ❌ | 5 | 20 |

La interfaz estructurada NO consume IA.
Solo el Lienzo IA y los flujos y visualizaciones personalizadas consume según tu uso.

---

## DIAPOSITIVA 21: APEX — ¿QUÉ PLAN ELEGIR?

APEX: ¿Qué Plan para Cada Rol?

| Si tu equipo... | Plan | Precio EUR | Precio MXN |
|-----------------|------|------------|------------|
| Solo necesita registrar y consultar datos | BASE | 9€/mes | $180/mes |
| Quiere dictar visitas por voz y recibir sugerencias | ESENCIAL | 19€/mes | $380/mes |
| Necesita briefings automáticos y IA proactiva | PRO | 29€/mes | $580/mes |

EJEMPLO ABOVE PHARMA (10 usuarios):

- 6 reps en ESENCIAL (19€/$380) → dictan visitas por voz
- 2 supervisores en PRO (29€/$580) → reciben briefings y alertas
- 2 admin en BASE (9€/$180) → solo consultan datos

TOTAL: 6×19 + 2×29 + 2×9 = 190€/mes ($3,800 MXN/mes)

---

## DIAPOSITIVA 22: APEX — COMPARATIVA CON EL MERCADO

APEX vs La Competencia

| Sistema | Plan con IA básica | Plan con IA completa |
|---------|-------------------|---------------------|
| APEX | 19€ ($380 MXN)/usuario | 29€ ($580 MXN)/usuario |
| Freshsales | 39€ ($780 MXN)/usuario | 59€ ($1,180 MXN)/usuario |
| Zoho | 40€ ($800 MXN)/usuario | 52€ ($1,040 MXN)/usuario |
| Pipedrive | 50€ ($1,000 MXN)/usuario | 75€ ($1,500 MXN)/usuario |
| HubSpot | 100€ ($2,000 MXN)/usuario | 150€ ($3,000 MXN)/usuario |

APEX es ~50% más económico.

¿Por qué?
- APEX ya existe, solo lo configuramos para ti
- Otros te construyen desde cero (15.000-50.000€ / $300,000-$1,000,000 MXN)
- La IA está integrada, no es un módulo adicional

---

## DIAPOSITIVA 23: OMIA — ¿QUÉ ES?

OMIA
Asistente IA de Ventas

OMIA es tu copiloto inteligente de ventas.

No reemplaza a tus vendedores.
Los hace MÁS EFECTIVOS.

Imagina que cada vendedor tiene un experto en productos sentado a su lado, 24/7.

Funciona por voz: "Hola, Omia..."
Funciona por texto en el celular.
Con TU información de productos.

La plataforma YA EXISTE — solo la personalizamos para ti.

---

## DIAPOSITIVA 24: OMIA — ¿CÓMO FUNCIONA?

OMIA: El Escenario Real

Tu vendedor está frente a un médico.
El médico hace una pregunta difícil.

VENDEDOR (por voz):
"Omia, el doctor pregunta por qué MyFiller es mejor que Juvederm"

OMIA responde en 5 segundos:
"MyFiller ofrece 3 ventajas clave:
1. Mayor duración (18 meses vs 12)
2. Menor inflamación post-procedimiento
3. Precio 20% más competitivo
[con estudios clínicos de respaldo]"

El vendedor cierra la venta con CONFIANZA.

---

## DIAPOSITIVA 25: OMIA — FUNCIONALIDADES

OMIA: Qué Puede Hacer

BASE DE CONOCIMIENTO
Todos tus productos, fichas técnicas, precios, indicaciones, contraindicaciones.
La información que hoy está en PDFs que nadie lee.

MANEJO DE OBJECIONES
"Es muy caro" → argumentos preparados
"No conozco la marca" → evidencia clínica
"Ya uso otro" → comparativa objetiva

COMPARATIVAS VS COMPETENCIA
Juvederm, Restylane, Radiesse, Sculptra
Con datos reales, no opiniones.

INTERFAZ DUAL
Por VOZ ("Hola Omia...") o por TEXTO
App móvil (PWA) instalable en el celular

---

## DIAPOSITIVA 26: OMIA — PARA ABOVE PHARMA

OMIA para Above Pharma

Personalizamos OMIA con TU contenido:

PRODUCTOS
MyFiller y toda tu línea actual y futura

OBJECIONES
Las que TU equipo enfrenta en campo

COMPETENCIA
vs Juvederm, Restylane, Radiesse, etc.

ARGUMENTOS DE VENTA
Por perfil: dermatólogos, cirujanos plásticos, medicina estética

---

## DIAPOSITIVA 27: OMIA — PLATAFORMA DE GESTIÓN

OMIA: Plataforma de Gestión para el Manager

No solo es un asistente para vendedores.
También tienes una PLATAFORMA DE GESTIÓN:

| Funcionalidad | PROFESIONAL | ENTERPRISE |
|---------------|-------------|------------|
| Panel de control | ✓ | ✓ |
| Ver actividad de usuarios | ✓ | ✓ |
| Ver consultas realizadas | ✓ | ✓ |
| Gestión de usuarios (alta/baja) | ✓ | ✓ |
| Estadísticas de uso básicas | ✓ | ✓ |
| Analytics avanzados | — | ✓ |
| Reportes exportables | — | ✓ |
| Métricas por usuario/equipo | — | ✓ |
| Alertas de uso | — | ✓ |
| Auditoría completa | — | ✓ |

Si tus vendedores preguntan mucho sobre "precio vs Juvederm", sabes que necesitan más entrenamiento en esa área.

OMIA te da INTELIGENCIA sobre tu equipo.

---

## DIAPOSITIVA 28: OMIA — DESARROLLO INICIAL

OMIA: Desarrollo Inicial

┌───────────────────────────────────────────────────────────────┐
│  Precio normal:           1.000€ / $20,000 MXN                │
│  OFERTA ESPECIAL:           500€ / $10,000 MXN                │
│  (50% descuento por lanzamiento)                              │
└───────────────────────────────────────────────────────────────┘

QUÉ INCLUYE EL DESARROLLO:

- Configuración de tu cuenta y usuarios
- Personalización de interfaz (logo, colores corporativos)
- Carga de información de productos (fichas técnicas, precios)
- Carga de objeciones comunes y respuestas
- Carga de comparativas vs competencia
- Entrenamiento inicial del modelo con tu contenido
- Pruebas de calidad de respuestas
- Capacitación al equipo (1-2 horas)
- Acompañamiento primera semana

Tiempo de desarrollo: 3-5 días hábiles

---

## DIAPOSITIVA 29: OMIA — PLANES DE SUSCRIPCIÓN

OMIA: Los 2 Planes de Suscripción

| Característica | PROFESIONAL 99€/mes ($1,980 MXN) | ENTERPRISE 349€/mes ($6,980 MXN) |
|----------------|----------------------------------|----------------------------------|
| Usuarios incluidos | 10 | 50 |
| Usuario adicional | +20€/mes ($400 MXN) | +5€/mes ($100 MXN) |
| Productos cargados | 50 | Ilimitados |
| Consultas/mes | 3.000 | 10.000 |
| Chat por texto | ✓ | ✓ |
| Interfaz de voz | ✓ | ✓ |
| PWA (app móvil) | ✓ | ✓ |
| Plataforma de gestión | Básica | Completa |
| Sistema de agentes IA | — | ✓ |
| Integración con sistemas | — | 1 incluida |
| Analytics/métricas | Básico | Avanzado |
| Soporte | WhatsApp | Prioritario + llamadas |

Los 2 primeros meses de suscripción son GRATIS

---

## DIAPOSITIVA 30: OMIA — EJEMPLO ABOVE PHARMA

OMIA para Above Pharma: Tu Inversión

PLAN RECOMENDADO: PROFESIONAL

| Concepto | EUR | MXN |
|----------|-----|-----|
| Desarrollo inicial (con oferta 50%) | 500€ | $10,000 |
| 2 primeros meses de suscripción | GRATIS | GRATIS |
| Suscripción mensual (10 usuarios) | 99€/mes | $1,980/mes |
| **INVERSIÓN AÑO 1** | **1.490€** | **$29,800** |

Incluye:
- 10 usuarios
- 3.000 consultas/mes
- Voz + texto
- PWA instalable
- Plataforma de gestión básica
- Soporte WhatsApp

Si necesitan más usuarios:
- 15 usuarios: 99€ + (5×20€) = 199€/mes ($3,980 MXN/mes)
- 20 usuarios: 99€ + (10×20€) = 299€/mes ($5,980 MXN/mes)

---

## DIAPOSITIVA 31: CATÁLOGO DE SOLUCIONES IA

Y ESTO ES SOLO EL INICIO...

PRISMA tiene un catálogo completo de soluciones de IA para tu negocio:

- Chatbots para atención al cliente
- Agentes IA autónomos
- Analytics e inteligencia de negocio
- Automatización de documentos
- IA para eventos y conferencias
- Generación de contenido

35% DE DESCUENTO en nuevas soluciones si ya tienes un producto contratado

La base de conocimiento de Above Pharma ya está creada.
Solo la reutilizamos para nuevas soluciones.

---

## DIAPOSITIVA 32: ¿POR QUÉ LOS 3 JUNTOS?

¿POR QUÉ LOS 3 PROYECTOS JUNTOS?

BRAND PLAN define la estrategia
(qué decir, a quién, cómo posicionarse)
↓
APEX automatiza procesos y experiencias
(visitas, médicos, reportes, KPIs)
↓
OMIA capacita y asiste al equipo
(argumentos, objeciones, comparativas)

= ECOSISTEMA COMPLETO ABOVE PHARMA

El Brand Plan define los mensajes.
APEX mide si funcionan.
OMIA los pone en boca de tu equipo.

Todo conectado. Todo medible.

---

## DIAPOSITIVA 33: RESUMEN DE INVERSIÓN

TU INVERSIÓN TOTAL

| PROYECTO | Precio Normal | Con Descuento | Ahorras |
|----------|---------------|---------------|---------|
| BRAND PLAN MyFiller (8 rubros) | 3.300€ / $66,000 | 3.300€ / $66,000 | — |
| APEX (Automatización de Procesos y EXperiencias) | 5.000€ / $100,000 | 2.500€ / $50,000 | **50% = 2.500€** |
| OMIA (Asistente IA Ventas) | 1.000€ / $20,000 | 500€ / $10,000 | **50% = 500€** |
| Subtotal | 9.300€ / $186,000 | 6.300€ / $126,000 | 3.000€ |
| Descuento paquete 3 proyectos | — | -630€ / -$12,600 | **10%** |
| **TOTAL PAQUETE INTEGRAL** | **9.300€** | **5.670€ / $113,400** | **3.630€ / $72,600** |

RESUMEN DE DESCUENTOS:
- APEX desarrollo: 50% descuento por lanzamiento (ahorras 2.500€ / $50,000 MXN)
- OMIA desarrollo: 50% descuento por lanzamiento (ahorras 500€ / $10,000 MXN)
- Paquete 3 proyectos: 10% descuento adicional (ahorras 630€ / $12,600 MXN)
- **AHORRO TOTAL: 3.630€ ($72,600 MXN)**

+ Suscripciones mensuales:
- APEX: desde 9€/usuario/mes ($180 MXN) según plan de IA
- OMIA: 99€/mes ($1,980 MXN) - 10 usuarios incluidos + 2 meses GRATIS

---

## DIAPOSITIVA 34: CONDICIONES COMERCIALES

CONDICIONES COMERCIALES

FORMA DE PAGO
50% al inicio | 50% a la entrega

MONEDA
EUR (referencia: $20 MXN/EUR)

INCLUYE
- Reuniones semanales de seguimiento
- 2 rondas de revisión por entregable
- Presentación ejecutiva
- Sesión de transferencia de conocimiento

EXCLUSIONES
- Producción de materiales gráficos
- Honorarios de terceros (KOLs)
- Suscripciones mensuales post-setup

Validez de esta propuesta: 30 días

---

## DIAPOSITIVA 35: PRÓXIMOS PASOS

PRÓXIMOS PASOS

1. Kick-off conjunto
   Recopilamos material para Brand Plan
   Completa el formulario de APEX (escanea el QR)

2. Desarrollo paralelo (4-6 semanas)
   Brand Plan + APEX + OMIA
   Reuniones semanales de avance

3. Entrega y capacitación
   Tu equipo listo para ejecutar

4. Crecimiento continuo
   Más soluciones IA con 35% descuento

Contacto: José Pablo Villaseñor
jp.villasenor@prismaconsul.com

---

## DIAPOSITIVA 36: RESUMEN FINAL

RESUMEN

3 PROYECTOS INTEGRADOS:

BRAND PLAN → Estrategia clara
APEX → Automatiza procesos y experiencias
OMIA → Tu equipo vende mejor

INVERSIÓN TOTAL: 5.670€ / $113,400 MXN
(Incluye 10% descuento por paquete)

Folio: PC26-0205 | Validez: 30 días

El detalle completo de cada proyecto se encuentra en los documentos anexos.

---

*© 2026 PRISMA Consulting. Todos los derechos reservados.*
