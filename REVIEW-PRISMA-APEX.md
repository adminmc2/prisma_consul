...
| Momento actual | Sprint A / Fase 2 en curso; subpasos 2.1 a 2.8 cerrados y validados (incl. migración aditiva de BD aplicada y validada en runtime sobre producción `v3.3.38`); subpaso 2.7 (`server/lib/domain-sync.js` skeleton) creado en `v3.3.39` como helper exportado y documentado pero **no invocado** desde rutas; subpaso 2.8 (refresh final de `CLAUDE.md` Directory Structure) cerrado en `v3.3.40`; único pendiente: 2.9 (export scripts ARMC a `prisma-consulting`) |
| Naturaleza del trabajo | Fase 2 con subpasos 2.1 a 2.8 absorbidos en `origin/dev`; 2.6 cerrado en runtime sobre producción; 2.7 helper skeleton no invocado; 2.8 cierre documental final de `CLAUDE.md` Directory Structure. Carril repo oficial = carpeta principal limpia sobre `dev`; carril contenido = worktree dedicado con handoff por SHA. Siguiente slice pendiente de autorización: 2.9 (export scripts ARMC a `prisma-consulting`) |
| Estado de aprobación | `MODELO-DOMINIO.md` aprobado; `ECOSISTEMA.md` alineado; `CONTRATOS.md` aprobado (C09 cerrado); `GLOSARIO.md` aprobado (C10 cerrado); bloques A, B, C y D cerrados; Fase 1 cerrada formalmente; subpasos 2.1 / 2.2 / 2.3 / 2.4 / 2.5 / 2.6 / 2.7 / 2.8 absorbidos en `origin/dev`; C12 cerrado; siguiente subpaso operativo: 2.9 (export scripts ARMC a `prisma-consulting`) — pendiente autorización |
| Condición de avance | Gate de Fase 2 cumplido (v3.2.44) y ya ejercido. **Bloque B PASS** (v3.3.3), **bloque C PASS** (v3.3.8) y **bloque D PASS** tras validación durable en `above-pharma`, `apex-agents`, `prisma-consulting` y `prisma-server-ops` el 2026-04-30. Reconciliación Git `main` → `dev` (merge `65c1301`) desplegada en `dev.prismaconsul.com` en `v3.3.21` y validada técnicamente. En `v3.3.22`: `origin/main` se promueve por fast-forward al baseline; producción queda alineada con `dev`. Subpasos físicos 2.1 a 2.5 absorbidos en `origin/dev` entre `v3.3.28` y `v3.3.37`; micro-higiene de serving en `v3.3.38`; subpaso 2.6 (migración aditiva de BD) ejecutado y validado en runtime sobre producción en `v3.3.38`; subpaso 2.7 (`server/lib/domain-sync.js` skeleton, no invocado) cerrado en `v3.3.39`; subpaso 2.8 (refresh final de `CLAUDE.md` Directory Structure) cerrado en `v3.3.40`. La carpeta principal local es la base oficial del carril repo; el carril contenido sigue pausado tras el último freeze autorizado (`50f39af`/`e7469f1`/`a5fca31`). Único slice pendiente de autorización: 2.9 (export scripts ARMC a `prisma-consulting`). |
