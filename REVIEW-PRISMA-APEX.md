...
| Momento actual | Sprint A / Fase 2 con los 9 subpasos físicos absorbidos en `dev`: 2.1 a 2.5 estructurales (`v3.3.28`–`v3.3.37`) + micro-higiene de serving (`v3.3.38`) + 2.6 migración aditiva de BD validada en runtime sobre producción (`v3.3.38`) + 2.7 helper `server/lib/domain-sync.js` skeleton no invocado (`v3.3.39`) + 2.8 refresh final de `CLAUDE.md` Directory Structure (`v3.3.40`) + 2.9 export de operación consultiva ARMC a `prisma-consulting` (`v3.3.41`, commit espejo `1e0ee7b` en `prisma-consulting/dev`). Pendientes operativos fuera de los subpasos físicos: replicación a producción de la estructura Fase 2 + nginx Variante B; cableado del helper `domain-sync.js` en rutas; reactivación del ejecutor 2 |
| Naturaleza del trabajo | Fase 2 con los 9 subpasos físicos (2.1 a 2.9) absorbidos en `origin/dev`; 2.6 cerrado en runtime sobre producción; 2.7 helper skeleton no invocado; 2.8 cierre documental final de `CLAUDE.md` Directory Structure; 2.9 export de operación consultiva ARMC a `prisma-consulting` (commit espejo `1e0ee7b` en `prisma-consulting/dev`). Carril repo oficial = carpeta principal limpia sobre `dev`; carril contenido = worktree dedicado con handoff por SHA. Trabajo restante fuera de los subpasos físicos: replicación a producción + cableado de `domain-sync.js` + reactivación del ejecutor 2 |
| Estado de aprobación | `MODELO-DOMINIO.md` aprobado; `ECOSISTEMA.md` alineado; `CONTRATOS.md` aprobado (C09 cerrado); `GLOSARIO.md` aprobado (C10 cerrado); bloques A, B, C y D cerrados; Fase 1 cerrada formalmente; subpasos 2.1 / 2.2 / 2.3 / 2.4 / 2.5 / 2.6 / 2.7 / 2.8 / 2.9 absorbidos en `origin/dev`; C12 cerrado. Subpasos físicos de Fase 2 completos en `dev`. Pendientes operativos posteriores: replicación a producción, cableado de `domain-sync.js`, reactivación del ejecutor 2 — todos requieren paquete específico |
| Condición de avance | Gate de Fase 2 cumplido (v3.2.44) y ya ejercido. **Bloque B PASS** (v3.3.3), **bloque C PASS** (v3.3.8) y **bloque D PASS** tras validación durable en `above-pharma`, `apex-agents`, `prisma-consulting` y `prisma-server-ops` el 2026-04-30. Reconciliación Git `main` → `dev` (merge `65c1301`) desplegada en `dev.prismaconsul.com` en `v3.3.21` y validada técnicamente. En `v3.3.22`: `origin/main` se promueve por fast-forward al baseline; producción queda alineada con `dev`. Subpasos físicos 2.1 a 2.5 absorbidos en `origin/dev` entre `v3.3.28` y `v3.3.37`; micro-higiene de serving en `v3.3.38`; subpaso 2.6 (migración aditiva de BD) ejecutado y validado en runtime sobre producción en `v3.3.38`; subpaso 2.7 (`server/lib/domain-sync.js` skeleton, no invocado) cerrado en `v3.3.39`; subpaso 2.8 (refresh final de `CLAUDE.md` Directory Structure) cerrado en `v3.3.40`; subpaso 2.9 (export de operación consultiva ARMC a `prisma-consulting`, commit espejo `1e0ee7b` en `prisma-consulting/dev`) cerrado en `v3.3.41`. La carpeta principal local es la base oficial del carril repo; el carril contenido sigue pausado tras el último freeze autorizado (`50f39af`/`e7469f1`/`a5fca31`). Subpasos físicos de Fase 2 completos en `dev`. El siguiente trabajo cambia de naturaleza: ya no son subpasos del plan físico sino paquetes operativos posteriores (replicación a producción, cableado de `domain-sync.js`, reactivación del ejecutor 2) — cada uno con su autorización propia. |
...
- Fase 2 con los 9 subpasos físicos absorbidos en `origin/dev` (no se reabren): 2.1 a 2.5 estructurales (`v3.3.28`–`v3.3.37`), 2.6 migración aditiva de BD validada en runtime sobre producción (`v3.3.38`), 2.7 helper `server/lib/domain-sync.js` skeleton no invocado (`v3.3.39`), 2.8 refresh final de `CLAUDE.md` Directory Structure (`v3.3.40`), 2.9 export de operación consultiva ARMC a `prisma-consulting` (`v3.3.41`, commit espejo `1e0ee7b` + corrección de paths/comandos en `efaf977` sobre `prisma-consulting/dev`). Subpasos físicos de Fase 2 completos en `dev`. Pendientes operativos posteriores (cada uno con su autorización propia): replicación a producción + cableado de `domain-sync.js` en rutas + reactivación del ejecutor 2.
