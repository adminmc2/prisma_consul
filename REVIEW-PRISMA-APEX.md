...
| Momento actual | Sprint A / Fase 2 con los 9 subpasos físicos absorbidos en `dev` y promovidos a `main`: 2.1 a 2.5 estructurales (`v3.3.28`–`v3.3.37`) + micro-higiene de serving (`v3.3.38`) + 2.6 migración aditiva de BD validada en runtime sobre producción (`v3.3.38`) + 2.7 helper `server/lib/domain-sync.js` skeleton no invocado (`v3.3.39`) + 2.8 refresh final de `CLAUDE.md` Directory Structure (`v3.3.40`) + 2.9 export de operación consultiva ARMC a `prisma-consulting` (`v3.3.41`, commit espejo `1e0ee7b` en `prisma-consulting/dev`). Producción sirviendo `v3.3.41`, nginx producción y dev alineados con Variante B, micro-paquete `404/410 text/plain` cerrado, checkout `~/web-de-prisma-dev` del VPS realineado a `b61d00f`. Pendientes operativos vigentes: cableado del helper `domain-sync.js` en rutas; reactivación del ejecutor 2 |
| Naturaleza del trabajo | Fase 2 con los 9 subpasos físicos (2.1 a 2.9) absorbidos en `origin/dev` y promovidos a `origin/main`; 2.6 cerrado en runtime sobre producción; 2.7 helper skeleton no invocado; 2.8 cierre documental final de `CLAUDE.md` Directory Structure; 2.9 export de operación consultiva ARMC a `prisma-consulting` (commit espejo `1e0ee7b` en `prisma-consulting/dev`). Producción ya replicada a Variante B y a la estructura Fase 2; micro-paquete nginx `404/410 text/plain` aplicado en dev y prod; higiene operativa del checkout VPS dev cerrada. Carril repo oficial = carpeta principal limpia sobre `dev`; carril contenido = worktree dedicado con handoff por SHA. Trabajo restante: cableado de `domain-sync.js` + reactivación del ejecutor 2 |
| Estado de aprobación | `MODELO-DOMINIO.md` aprobado; `ECOSISTEMA.md` alineado; `CONTRATOS.md` aprobado (C09 cerrado); `GLOSARIO.md` aprobado (C10 cerrado); bloques A, B, C y D cerrados; Fase 1 cerrada formalmente; subpasos 2.1 / 2.2 / 2.3 / 2.4 / 2.5 / 2.6 / 2.7 / 2.8 / 2.9 absorbidos en `origin/dev` y promovidos a `origin/main`; C12 cerrado. Subpasos físicos de Fase 2 completos en `dev` y producción. Pendientes operativos vigentes: cableado de `domain-sync.js`, reactivación del ejecutor 2 — cada uno requiere paquete específico |
| Condición de avance | Gate de Fase 2 cumplido (v3.2.44) y ya ejercido. **Bloque B PASS** (v3.3.3), **bloque C PASS** (v3.3.8) y **bloque D PASS** tras validación durable en `above-pharma`, `apex-agents`, `prisma-consulting` y `prisma-server-ops` el 2026-04-30. Reconciliación Git `main` → `dev` (merge `65c1301`) desplegada en `dev.prismaconsul.com` en `v3.3.21` y validada técnicamente. En `v3.3.22`: `origin/main` se promueve por fast-forward al baseline; producción queda alineada con `dev`. Subpasos físicos 2.1 a 2.5 absorbidos en `origin/dev` entre `v3.3.28` y `v3.3.37`; micro-higiene de serving en `v3.3.38`; subpaso 2.6 (migración aditiva de BD) ejecutado y validado en runtime sobre producción en `v3.3.38`; subpaso 2.7 (`server/lib/domain-sync.js` skeleton, no invocado) cerrado en `v3.3.39`; subpaso 2.8 (refresh final de `CLAUDE.md` Directory Structure) cerrado en `v3.3.40`; subpaso 2.9 (export de operación consultiva ARMC a `prisma-consulting`, commit espejo `1e0ee7b` en `prisma-consulting/dev`) cerrado en `v3.3.41`. El 2026-05-05 se promueve `dev` a `main` por fast-forward (`b61d00f`), se despliega producción a `v3.3.41`, se alinea nginx de producción a Variante B, se aplica el micro-paquete `404/410 text/plain` en dev y prod (validado en origen y públicamente) y se cierra la higiene operativa del checkout `~/web-de-prisma-dev` del VPS (alineado a `b61d00f`, vestigio `images/datos.mp4` eliminado tras verificar duplicado byte a byte de `web/images/videos/datos.mp4`). La carpeta principal local es la base oficial del carril repo; el carril contenido sigue pausado tras el último freeze autorizado (`50f39af`/`e7469f1`/`a5fca31`). El siguiente trabajo son paquetes operativos posteriores (cableado de `domain-sync.js`, reactivación del ejecutor 2) — cada uno con su autorización propia. |
...
- El estado de negocio efectivo del sistema legacy sigue dependiendo de `current_phase`, `profile_type` y `apex_submission_id`, mientras `domain-sync.js` permanece como capa skeleton aún no cableada en rutas.
...
- Próximo paso: el siguiente paquete autorizado es **cableado de `domain-sync.js` en rutas**; tras él, **reactivación del ejecutor 2**. Nada más queda pendiente del bloque post-Fase 2.
