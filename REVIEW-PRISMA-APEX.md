...
| Momento actual | Sprint A / Fase 2 con los 9 subpasos físicos absorbidos en `dev` y promovidos a `main`: 2.1 a 2.5 estructurales (`v3.3.28`–`v3.3.37`) + micro-higiene de serving (`v3.3.38`) + 2.6 migración aditiva de BD validada en runtime sobre producción (`v3.3.38`) + 2.7 helper `server/lib/domain-sync.js` skeleton no invocado (`v3.3.39`) + 2.8 refresh final de `CLAUDE.md` Directory Structure (`v3.3.40`) + 2.9 export de operación consultiva ARMC a `prisma-consulting` (`v3.3.41`, commit espejo `1e0ee7b` en `prisma-consulting/dev`). Producción sirviendo `v3.3.41`, nginx producción y dev alineados con Variante B, micro-paquete `404/410 text/plain` cerrado, checkout `~/web-de-prisma-dev` del VPS realineado a `b61d00f`. Cableado mínimo de `domain-sync.js` aplicado en `dev` (`v3.3.42`): `syncLegacyUserUpdate` deja de ser skeleton y queda invocado por `PATCH /api/portal-profile` y `PATCH /api/portal-users/:id` con CT-4 intacto. Pendientes operativos vigentes: deploy de `v3.3.42` a producción; reactivación del ejecutor 2 |
| Naturaleza del trabajo | Fase 2 con los 9 subpasos físicos (2.1 a 2.9) absorbidos en `origin/dev` y promovidos a `origin/main`; 2.6 cerrado en runtime sobre producción; 2.7 helper skeleton (cableado posterior en `v3.3.42`); 2.8 cierre documental final de `CLAUDE.md` Directory Structure; 2.9 export de operación consultiva ARMC a `prisma-consulting` (commit espejo `1e0ee7b` en `prisma-consulting/dev`). Producción ya replicada a Variante B y a la estructura Fase 2; micro-paquete nginx `404/410 text/plain` aplicado en dev y prod; higiene operativa del checkout VPS dev cerrada. Cableado mínimo de `domain-sync.js` ejecutado en `dev` (`v3.3.42`) con sincronización atómica y `profile_type` legacy-only por decisión explícita. Carril repo oficial = carpeta principal limpia sobre `dev`; carril contenido = worktree dedicado con handoff por SHA. Trabajo restante: aprobación de push de `v3.3.42` + deploy a producción + reactivación del ejecutor 2 |
| Estado de aprobación | `MODELO-DOMINIO.md` aprobado; `ECOSISTEMA.md` alineado; `CONTRATOS.md` aprobado (C09 cerrado); `GLOSARIO.md` aprobado (C10 cerrado); bloques A, B, C y D cerrados; Fase 1 cerrada formalmente; subpasos 2.1 / 2.2 / 2.3 / 2.4 / 2.5 / 2.6 / 2.7 / 2.8 / 2.9 absorbidos en `origin/dev` y promovidos a `origin/main`; C12 cerrado. Subpasos físicos de Fase 2 completos en `dev` y producción. Cableado mínimo de `domain-sync.js` cerrado en `dev` (`v3.3.42`) con CT-4 intacto. Pendientes operativos vigentes: deploy `v3.3.42` a producción, reactivación del ejecutor 2 — cada uno requiere paquete específico |
| Condición de avance | Gate de Fase 2 cumplido (v3.2.44) y ya ejercido. **Bloque B PASS** (v3.3.3), **bloque C PASS** (v3.3.8) y **bloque D PASS** tras validación durable en `above-pharma`, `apex-agents`, `prisma-consulting` y `prisma-server-ops` el 2026-04-30. Reconciliación Git `main` → `dev` (merge `65c1301`) desplegada en `dev.prismaconsul.com` en `v3.3.21` y validada técnicamente. En `v3.3.22`: `origin/main` se promueve por fast-forward al baseline; producción queda alineada con `dev`. Subpasos físicos 2.1 a 2.5 absorbidos en `origin/dev` entre `v3.3.28` y `v3.3.37`; micro-higiene de serving en `v3.3.38`; subpaso 2.6 (migración aditiva de BD) ejecutado y validado en runtime sobre producción en `v3.3.38`; subpaso 2.7 (`server/lib/domain-sync.js` skeleton, no invocado) cerrado en `v3.3.39`; subpaso 2.8 (refresh final de `CLAUDE.md` Directory Structure) cerrado en `v3.3.40`; subpaso 2.9 (export de operación consultiva ARMC a `prisma-consulting`, commit espejo `1e0ee7b` en `prisma-consulting/dev`) cerrado en `v3.3.41`. El 2026-05-05 se promueve `dev` a `main` por fast-forward (`b61d00f`), se despliega producción a `v3.3.41`, se alinea nginx de producción a Variante B, se aplica el micro-paquete `404/410 text/plain` en dev y prod (validado en origen y públicamente) y se cierra la higiene operativa del checkout `~/web-de-prisma-dev` del VPS (alineado a `b61d00f`, vestigio `images/datos.mp4` eliminado tras verificar duplicado byte a byte de `web/images/videos/datos.mp4`). En `v3.3.42` se ejecuta el cableado mínimo de `domain-sync.js` en los dos PATCH del portal con sincronización atómica y `profile_type` legacy-only por decisión explícita (commit local pendiente de aprobación de push). La carpeta principal local es la base oficial del carril repo; el carril contenido sigue pausado tras el último freeze autorizado (`50f39af`/`e7469f1`/`a5fca31`). Trabajo restante: aprobación de push del commit `v3.3.42` + deploy a producción + reactivación del ejecutor 2 — cada uno con su autorización propia. |
...
- Fase 2 con los 9 subpasos físicos absorbidos en `origin/dev` y promovidos a `origin/main` por fast-forward el 2026-05-05 (no se reabren): 2.1 a 2.5 estructurales (`v3.3.28`–`v3.3.37`), 2.6 migración aditiva de BD validada en runtime sobre producción (`v3.3.38`), 2.7 helper `server/lib/domain-sync.js` skeleton no invocado (`v3.3.39`), 2.8 refresh final de `CLAUDE.md` Directory Structure (`v3.3.40`), 2.9 export de operación consultiva ARMC a `prisma-consulting` (`v3.3.41`, commit espejo `1e0ee7b` + corrección de paths/comandos en `efaf977` sobre `prisma-consulting/dev`). Producción sirviendo `v3.3.41`; nginx de producción alineado con Variante B (replicado de dev); micro-paquete nginx `404/410 text/plain` aplicado en dev y prod (`@plain404` named location, `/apex/fonts/*` con `410 text/plain`, misses bajo `/apex` y `/hub` con `404 text/plain`), validado en origen y públicamente; higiene operativa del checkout `~/web-de-prisma-dev` del VPS cerrada (HEAD = `b61d00f`, status limpio, vestigio `images/datos.mp4` eliminado tras verificar duplicado byte a byte de `web/images/videos/datos.mp4`, ruta canónica `/images/videos/datos.mp4` operativa, ruta vestigial `/images/datos.mp4` devuelve 404). Cableado mínimo de `domain-sync.js` ejecutado en `dev` (`v3.3.42`) con sincronización atómica y `profile_type` legacy-only por decisión explícita (commit local pendiente de aprobación de push). Pendientes operativos vigentes (cada uno con su autorización propia): aprobación de push de `v3.3.42` + deploy a producción + reactivación del ejecutor 2.
...
- Próximo paso: el siguiente paquete autorizado es **cableado mínimo de `domain-sync.js` en los dos PATCH del portal** (ya ejecutado en `v3.3.42`, pendiente de aprobación de push y deploy a producción); tras él, **reactivación del ejecutor 2**. Nada más queda pendiente del bloque post-Fase 2.
...
- Próximo paso: aprobación de push del commit local + deploy de `v3.3.42` a producción + reactivación del ejecutor 2 (cada uno con su autorización propia).
