...
- Fase 2 en curso: subpasos 2.1 a 2.8 ya absorbidos en `origin/dev` (no se reabren). 2.6 cerrado en runtime sobre producción (`v3.3.38`); 2.7 (`server/lib/domain-sync.js`) cerrado como skeleton no invocado (`v3.3.39`); 2.8 (refresh final de `CLAUDE.md` Directory Structure) cerrado en `v3.3.40`. Siguiente slice pendiente de autorización: 2.9 (export de scripts ARMC a `prisma-consulting`).
