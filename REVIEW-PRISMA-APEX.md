...
| Momento actual | Sprint A / Fase 2 con los 9 subpasos físicos absorbidos en `dev` y promovidos a `main`: 2.1 a 2.5 estructurales (`v3.3.28`–`v3.3.37`) + micro-higiene de serving (`v3.3.38`) + 2.6 migración aditiva de BD validada en runtime sobre producción (`v3.3.38`) + 2.7 helper `server/lib/domain-sync.js` skeleton no invocado (`v3.3.39`) + 2.8 refresh final de `CLAUDE.md` Directory Structure (`v3.3.40`) + 2.9 export de operación consultiva ARMC a `prisma-consulting` (`v3.3.41`, commit espejo `1e0ee7b` en `prisma-consulting/dev`). Producción sirviendo `v3.3.42`, nginx producción y dev alineados con Variante B, micro-paquete `404/410 text/plain` cerrado, checkout `~/web-de-prisma-dev` del VPS alineado a `f7d335d4` / `origin/dev` (deploy DEV de `v3.3.42`), limpio. Cableado mínimo de `domain-sync.js` aplicado y promovido a `main` (`v3.3.42`; SHA `f7d335d4`): `syncLegacyUserUpdate` deja de ser skeleton y queda invocado por `PATCH /api/portal-profile` y `PATCH /api/portal-users/:id` con CT-4 intacto. `prismaconsul.com` y `/hub` validados públicamente en `v3.3.42`. Pendiente operativo vigente: reactivación del ejecutor 2 |
| Naturaleza del trabajo | Fase 2 con los 9 subpasos físicos (2.1 a 2.9) absorbidos en `origin/dev` y promovidos a `origin/main`; 2.6 cerrado en runtime sobre producción; 2.7 helper skeleton (cableado posterior en `v3.3.42`); 2.8 cierre documental final de `CLAUDE.md` Directory Structure; 2.9 export de operación consultiva ARMC a `prisma-consulting` (commit espejo `1e0ee7b` en `prisma-consulting/dev`). Producción ya replicada a Variante B y a la estructura Fase 2; micro-paquete nginx `404/410 text/plain` aplicado en dev y prod; higiene operativa del checkout VPS dev cerrada. Cableado mínimo de `domain-sync.js` ejecutado y promovido a `origin/main` (`v3.3.42`) con sincronización atómica y `profile_type` legacy-only por decisión explícita. Carril repo oficial = carpeta principal limpia sobre `dev`; carril contenido = worktree dedicado con handoff por SHA. Trabajo restante: reactivación del ejecutor 2 |
| Estado de aprobación | `MODELO-DOMINIO.md` aprobado; `ECOSISTEMA.md` alineado; `CONTRATOS.md` aprobado (C09 cerrado); `GLOSARIO.md` aprobado (C10 cerrado); bloques A, B, C y D cerrados; Fase 1 cerrada formalmente; subpasos 2.1 / 2.2 / 2.3 / 2.4 / 2.5 / 2.6 / 2.7 / 2.8 / 2.9 absorbidos en `origin/dev` y promovidos a `origin/main`; C12 cerrado. Subpasos físicos de Fase 2 completos en `dev` y producción. Cableado mínimo de `domain-sync.js` cerrado en `origin/dev` y `origin/main` (`v3.3.42`) con CT-4 intacto. `prismaconsul.com` y `/hub` validados en `v3.3.42`. Pendiente operativo vigente: reactivación del ejecutor 2 |
