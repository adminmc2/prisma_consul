...
- **Envelope de evento:** los cinco campos comunes que toda instancia emitida de evento del dominio ARMC lleva en la raíz: `event_id`, `event_type`, `subject_id`, `lead_id`, `occurred_at`. Separado del payload de negocio anidado.
- **`event_id`:** identificador de la fila de evento en `armc_events`. La PK física en la tabla sigue llamándose `id`; su representación en el envelope emitido se llama `event_id`. Distinto de `subject_id` y `lead_id`.
- **`occurred_at`:** instante en que ocurrió el evento. En el alcance actual coincide con el momento del INSERT en `armc_events` / `armc_handoffs`. La distinción entre "cuándo ocurrió" y "cuándo se insertó" queda preparada para futuros backfills o event replay.
- **Payload de negocio:** datos específicos del evento anidados en el campo `payload` de la instancia emitida. No repite campos del envelope.
- **Política de reconocimiento del subject (S4):** algoritmo contractual que decide, en el instante de captación inicial, si el contacto entrante crea un nuevo `subject_id`, reutiliza uno existente, o produce una incidencia. Detalle completo en `data-dictionary.md` sección `armc_subject_identifiers`. Regla base: normalizar identifiers, buscar candidatos activos con igual peso de PHONE y EMAIL, comparar nombres contra el episodio más reciente del subject candidato mediante normalización estricta (sin fuzzy).
- **Candidato de reconocimiento:** subject cuyo identifier activo coincide (por valor normalizado) con un identifier del contacto entrante. Un match produce candidato, no fusión automática.
- **Coincidencia dudosa (incidencia):** situación en la que hay múltiples candidatos, identifiers del contacto que apuntan a subjects distintos, divergencia nominal tras normalización estricta contra el episodio más reciente, o candidato sin episodio previo comparable. **Resultado contractual terminal**: no crea subject, no crea lead, no asocia identifiers, no emite eventos S3. Deriva a resolución humana futura (capacidad no implementada en S4).
- **Estados del identifier:** **normalizado** (forma canónica para lookup), **declarado o confirmado** (recibido y aceptado por el sistema; `verified_at IS NULL`), **verificado** (comprobado por mecanismo real — OTP, enlace; `verified_at` presente). En el alcance actual del simulador no hay mecanismo de verificación implementado; `verified_at` queda mayoritariamente NULL.
- **Contexto conversacional contractual persistente:** referencia contractual que conserva `subject_id + lead_id` durante un Flow WhatsApp ya establecido (post-`LEAD_CREATED`). S4 describe el comportamiento; la implementación técnica (almacén de sesiones, backend) queda para materialización futura. Elimina la resolución repetida por teléfono en cada paso del Flow.
- **Recurrencia:** nueva captación confirmada del mismo subject → nuevo `lead_id`. Los episodios pueden coexistir. S4 no reabre ni actualiza episodios previos por recurrencia. La distinción entre "recurrencia externa" (nueva captación) y "continuidad interna del Flow" (mismo contexto persistente) evita crear otro episodio por continuidad interna del Flow.
