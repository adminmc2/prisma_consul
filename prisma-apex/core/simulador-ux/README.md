...
- **Capa 3:** tres tablas — `armc_leads` (con columna `estado_actual` enum `LEAD_ABIERTO / LEAD_CONFIRMADO`; `demanda_ids_seleccionados` acepta `NULL` con CHECK condicional que exige cardinalidad ≥ 1 solo en `LEAD_CONFIRMADO`; más las seis columnas de handoff aditivas: `handoff_state`, `handoff_assigned_to`, `handoff_close_reason`, `handoff_requested_at`, `handoff_assigned_at`, `handoff_closed_at`), `armc_events` (enum `event_type` ampliado a los cinco valores del alcance: `LEAD_CREATED`, `LEAD_CAPTURED`, y los tres del handoff) y `armc_handoffs` (historial completo del handoff: una fila por transición `REQUESTED / ASSIGNED / CLOSED`). FKs hacia `portal_users(id)` para identidad de humano. **En canal WhatsApp la ficha del lead se escribe en dos instantes**: `INSERT` tras confirmación explícita del lead (evento `LEAD_CREATED`, estado `LEAD_ABIERTO`) y `UPDATE` al enviar el Flow completo (evento `LEAD_CAPTURED`, estado `LEAD_CONFIRMADO`). En canal web se escribe en un único `INSERT` con estado `LEAD_CONFIRMADO` (evento `LEAD_CAPTURED`).
