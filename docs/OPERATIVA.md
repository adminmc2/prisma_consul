...
|---|---|---|
| nginx `dev`: `/publicados/armc/simulador-ux/...` se sirve como estático (en lugar del `301 → /hub` declarado por Express en `server.js`). Override de edge, no del repo. | 2026-05-21 | Reintroducir el `301` de Express cuando se confirme que ninguna página cacheada en dev consuma aún la ruta legacy. |
| nginx `prod`: `/publicados/armc/simulador-ux/...` se sirve como estático (mismo override que dev). El simulador nativo `/core/simulador-ux/*` es **operativo en producción**, estado verificado el 2026-08-02. | 2026-08-02 | Reintroducir el `301` de Express cuando se confirme que ninguna página cacheada en prod consuma aún la ruta legacy. |
