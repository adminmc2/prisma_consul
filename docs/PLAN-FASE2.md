...
> **Nota operativa (actualizada 2026-05-03, HEAD publicado `v3.3.29`).** Fase 2 **ya está en curso**: el subpaso 2.1 quedó absorbido en `origin/dev` y validado en `dev.prismaconsul.com`. El siguiente subpaso permitido es **2.2**. El modo operativo vigente simplificado es el definido en `CLAUDE.md`: la **carpeta principal limpia y alineada a `origin/dev`** funciona como base oficial del carril repo sobre `dev`; el carril contenido opera en un **worktree dedicado** sobre rama local real. Si un subpaso estructural mueve un subtree activo del carril contenido, el ejecutor 2 se pausa, se congelan los SHAs aprobados y el carril repo los absorbe antes del movimiento físico. Los worktrees extra quedan solo para auditoría, experimento o recuperación temporal.
